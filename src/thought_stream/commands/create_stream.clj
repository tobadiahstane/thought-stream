...
(defn create-stream [base-aggregate new-stream]
  ...
  (let [stream-id (:id new-stream)
        ...
        name-validation (if-not (util/valid-text? stream-name) (str "Invalid stream name: " stream-name " "))
        focus-text-validation (if-not (util/valid-text? focus-text) (str "Invalid focus text: " focus-text " "))]
    (if-not (and (nil? id-validation) (nil? name-validation) (nil? focus-text-validation))
      (throw (IllegalArgumentException. (s/trim (str "Invalid new stream: " id-validation name-validation focus-text-validation)))))
